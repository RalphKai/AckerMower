...
class Self_odom():
	...
	def call_back_cmd(self, cmd):	
		...
		rospy.loginfo("odomcallback_cmd_vel_angular.z:"+ str(self.z))
		...
		if self.cmd_x>0 and self.rpm>0:
			...
			self.vx = self.cmd_x / 2

		else:
			self.vx = 0

		if self.z>0:
			steering = self.convert_trans_rot_vel_to_steering_angle(self.cmd_x, self.z, 0.284)
