...
class drive():
	def __init__(self):
		self.rpm, self.left_ultra, self.right_ultra, self.forward_value = 0, 45, 45, 150
		self.forward_left_value, self.forward_right_value = 45, 45
...
# + ", "+ str(self.forward_value)+ ", "+ str(self.forward_right_value)+ ", "+ str(self.forward_left_value))   
	...
	def walltracking(self):
		error = 0
		rospy.loginfo("flag num: " + str(self.flag))
		if self.flag >= 2:
			if self.left_ultra < 40 and self.left_ultra > 10:
				self.forward = self.forward_forward		
				error = self.left_ultra - 25.0
				self.integral = self.integral + error
				self.derivative = error - self.lastError
				turn = self.Kp * error + self.Ki * (self.integral) + self.Kd * (self.derivative)
				rospy.loginfo("turn from Kp: "+ str(turn))
				self.angular = self.angular_init + turn
				if self.angular <= 0:
					self.angular = 0
				self.unit_publish()
				self.flag = 0
			...
			elif self.right_ultra < 40 and self.right_ultra > 10:
				self.forward = self.forward_forward
				error = -(self.right_ultra - 25.0)
				self.integral = self.integral + error
				self.derivative = error - self.lastError
				turn = self.Kp * (error) + self.Ki * (self.integral) + self.Kd * (self.derivative)
				rospy.loginfo("turn from Kp: "+ str(turn))
				...
				if self.forward == self.forward_init:
					pass
					
				else:
					# rospy.loginfo(self.forward)
					self.init_state()
					self.unit_publish()
